 ...
 *
 * Tests use this to *observe* — was the user's #__user_usergroup_map really emptied by the wipe? — and to
 * seed fixtures. They never use it to assert what an HTTP request should have proved.
 *
 * @since 4.1.0
 */
class Database
{
	...
	public function __construct(?Configuration $config = null)
	{
		...
		$dsn = sprintf(
			...
			(int) $config->get('db.port', 33310),
			$config->get('db.name', 'dce2e')
		);
		...
		try
		{
			$this->pdo = new PDO(
				$dsn,
				(string) $config->get('db.user', 'dce2e'),
				(string) $config->get('db.pass', 'dce2e'),
				[
