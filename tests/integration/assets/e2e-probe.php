...
/**
 * Identity probe for the Data Compliance end-to-end test suite.
 *
 * SiteProvisioner copies this file into the throwaway site's document root, substituting the shared
 * secret. It boots the real front-end application far enough to read the session's identity, then
 * answers with JSON. It never routes or dispatches, so it cannot have side effects on the site.
 *
 * Why this exists: the suite has to be able to say "this surfer is logged in as X" and "X holds
 * these privileges" without parsing a rendered template, which would differ between Joomla 5.4 and
 * 6.1 and break for reasons that have nothing to do with Data Compliance.
 *
 ...
 * the run, and it is never part of any Data Compliance package. The shared secret is there so a stray request
...
const DC_E2E_PROBE_SECRET = '##SECRET##';

header('Content-Type: application/json; charset=utf-8');
header('Cache-Control: no-store');

if (!hash_equals(DC_E2E_PROBE_SECRET, (string) ($_GET['secret'] ?? '')))
{
...
// Optional authorisation matrix: ?assets=com_datacompliance&actions=export,wipe,core.admin,core.manage
